...
const char *EDecoder::processCompletedOrdersEndMsg(const char *ptr, const char *)
{
...
bool EDecoder::DecodeField(double &doubleValue, const char *&ptr, const char *endPtr)
{
    ...
    if(!fieldEnd)
        return false;
    doubleValue = QString(fieldBeg).toDouble();//   atof(fieldBeg);
    ptr = ++fieldEnd;
    return true;
}
...
bool EDecoder::DecodeFieldMax(double &doubleValue, const char *&ptr, const char *endPtr)
{
    ...
    if(!DecodeField(stringValue, ptr, endPtr))
        return false;
    doubleValue = stringValue.empty() ? UNSET_DOUBLE : QString(stringValue.c_str()).toDouble();
    return true;
}
