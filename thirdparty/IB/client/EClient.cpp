...
void EClient::placeOrder(OrderId id, const Contract &contract, const Order &order)
{
    ...
    if(m_serverVersion >= MIN_SERVER_VER_PEGGED_TO_BENCHMARK) {
        if(order.orderType == "PEG BENCH") {
            ...
            ENCODE_FIELD_MAX(order.peggedChangeAmount);
            ENCODE_FIELD_MAX(order.referenceChangeAmount);
        ...
        ENCODE_FIELD_MAX(order.triggerPrice);
        ENCODE_FIELD_MAX(order.lmtPriceOffset);
        ENCODE_FIELD_MAX(order.adjustedStopPrice);
        ENCODE_FIELD_MAX(order.adjustedStopLimitPrice);
        ENCODE_FIELD_MAX(order.adjustedTrailingAmount);
