...
protected:
    ...
    struct CSize {
        ...
        //   Initialization
        void Set(unsigned int _x, unsigned int _y)
        {
            x = _x;
            y = _y;
        }
    ...
    struct CExtension: public CArray {
        ...
        ~CExtension()
        {
            if(CArray::Buffer) delete[] CArray::Buffer;
        }
    ...
    struct CWindow {
        ...
        //   Default constructor
        CWindow(): Weights(nullptr), Size(0), Correction(.5 - double(T(.5))) {}
        //   Destructor
        ~CWindow()
        {
    ...
    {
        return  _Image && _Size.x && _Size.y && _W && _Size.x > (_W >> 1) && _Size.y > (_W >> 1) && _W & 1;
    }
