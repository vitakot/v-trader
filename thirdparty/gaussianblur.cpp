...
template <class T> bool TGaussianBlur<T>::CExtension::_Allocate(const CSize &_Size, unsigned int _Margin)
{
    ...
    CArray::Buffer = new T[(_Size.x > _Size.y ? _Size.x : _Size.y) + (_Margin << 1)];
...
template <class T> void TGaussianBlur<T>::CExtension::Paste(const T *const _Start)
{
    if(Mode == ModeHorizontal) {
        //   Paste line
        memcpy(CArray::Buffer + Margin, _Start, CArray::Size.x * sizeof(T));
    }
    else {
        //   Stop position
        const T *const Stop = _Start + CArray::Size.Area();
        //   Array iterator
        const T *ArrIter = _Start;
        //   Extension iterator
        T *ExtIter = CArray::Buffer + Margin;
        //   Paste array column element by element
        while(ArrIter < Stop) {
            //   Copy line
            *(ExtIter++) = *ArrIter;
            //   Jump to the next line
            ArrIter += CArray::Size.x;
        }
    }
}
...
{
    //   Line size
    const unsigned int Line = Mode == ModeHorizontal ? CArray::Size.x : CArray::Size.y;
    //   Stop position
    const T *const Stop = CArray::Buffer - 1;
    //   Left extension iterator
    T *ExtLeft = CArray::Buffer + Margin - 1;
    //   Left array iterator
